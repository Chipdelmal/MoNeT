...
def draw_dots(ms,alphas, X=0, Y=0):
    N = len(alphas)
    start = 0.0
    for i in range(N):
        alphaValue = alphas[i]
        ms[i].scatter([X],[Y], latlon=True, marker='.', s=3, facecolor=colors[i], alpha= alphaValue)
...
for expPath in sorted(glob.glob(folder+'ANALYZED/E_*')):
    ...
    for time in range(lines):
        for i in range(patches):
            ...
            draw_dots(ms, alphas, longs[i], lats[i])

        else:
            for j in range(len(groups)):
                figs[j].savefig(subfolder+experiment+'/'+str(time).zfill(6)+'_'+groups[j]+".png",
