...
### Landscape ########################################################################################
######################################################################################################
heterogFile=read.csv(paste0("/Users/Biyonka/Desktop/Output/spaHet/spahet_files/","base_20m_50n.csv"),sep=",",header=TRUE)
...
for(j in 1:length(repFolders)){
  MGDrivEv2::deterministic_single(cubeR = driveCube,
                                  parametersR = netPar,
                                  migrationFemaleR = landscape$movementKernel,
                                  migrationMaleR = landscape$movementKernel,
                                  releasesR = patchReleases,output = repFolders[j],
                                  verbose = TRUE)
}
